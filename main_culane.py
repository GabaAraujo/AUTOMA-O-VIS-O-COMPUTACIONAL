...
class LaneCarDistanceSystem:
    def __init__(self, use_ufld=True, use_gpu=True):
        ...
        self.num_lanes = 4
        self.num_points = 100
        self.griding_num = 200  # 200 para Culane
    ...
    def _init_ufld(self):
        ...
        try:
            ...
            self.ufld_model = parsingNet(
                pretrained=False, 
                backbone='18',
                cls_dim=(self.griding_num + 1, 18, self.num_lanes),  # 18 para Culane
                use_aux=False
            ).to(self.device)
            
            # Carregar pesos pré-treinados
            weight_file = 'culane_r18.pth'
...
2. Baixe o arquivo culane_r18.pth da seção de releases
    ...
    def detect_cars(self, frame):
        ...
        if results[0].boxes is not None:
            for box in results[0].boxes:
                ...
                if confidence > 0.7:  # Filtrar detecções com baixa confiança
                    cars.append({
            ...
            if np.sum(out_j[:, i] != 0) > 2:
                for k in range(out_j.shape[0]):
                    if out_j[k, i] > 0:
                        ...
                        y_coord = int(orig_h * (culane_row_anchor[18-1-k]/288)) - 1
...
def main():
    ...
    try:
        ...
        print("2. Arquivo culane_r18.pth está presente")
